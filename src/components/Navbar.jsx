...
function Navbar() {
    ...
    return (
        <nav
            className={`fixed top-0 left-0 w-full  text-white px-20 py-8 flex justify-between items-center z-50 transition-opacity ${visible ? 'opacity-100' : 'opacity-0'
                }`}
        >
            <div className='logo'>
            ...
            <div className='links flex gap-10'>
                {["Services", "Our Work", "About Us", "Insights", "contacts"].map((item, index) => (<a className={`font-[Neue Montreal] text-md font-light capitalize ${index === 4 && "ml-32"}`} key={index}>{item}</a>))}
            </div>
        </nav>
